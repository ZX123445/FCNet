        ...
        for dirpath, dirnames, filenames in os.walk(image_root):
            for filename in filenames:
                if filename.endswith('.jpg') or filename.endswith('.png'):
                    ...
                    # 类似地，遍历 gt_root 目录下的所有文件夹及其文件
        for dirpath, dirnames, filenames in os.walk(gt_root):
            for filename in filenames:
                if filename.endswith('.jpg') or filename.endswith('.png'):
                    # 构建标签文件的完整路径
...
class test_dataset:
    ...
    def load_data(self):
        ...
        image_path_split = self.images[self.index].split('/')
        group_name = image_path_split[-2]  # 获取组名

        name = image_path_split[-1]
        ...
        return image, gt, name, group_name, np.array(image_for_post)
        """image = self.rgb_loader(self.images[self.index])
        image = self.transform(image).unsqueeze(0)
